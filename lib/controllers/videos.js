...
exports.create = function(req, res) {
  ...
  if (!videoId) return res.send(400, {videoUrl: ['cannot parse this youtube url ' + videoUrl]})
  return youtubeVideoSvc.getVideoMetadata(videoId, function(err, data) {
    if (err) return res.send(500, err)
    if (!data) return res.send(404, 'no video with id ' + videoId)
