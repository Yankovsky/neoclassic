...
exports.allOfType = function(type, callback) {
  db.query({
    ...
    text: 'SELECT id, slug, title, html, short_text AS "shortText", EXTRACT(epoch FROM datetime) * 1000 AS "datetime", thumbnail_url AS "thumbnailUrl", type FROM entries WHERE type = $1 ORDER BY datetime DESC',
    values: [type]
  }, callback)
}
...
exports.get = function(id, callback) {
  db.single({
    ...
    text: 'SELECT id, slug, title, html, short_text AS "shortText", EXTRACT(epoch FROM datetime) * 1000 AS "datetime", thumbnail_url AS "thumbnailUrl", type FROM entries WHERE id = $1',
    values: [id]
  }, callback)
}
...
exports.getByTypeAndSlug = function(type, slug, callback) {
  db.single({
    ...
    text: 'SELECT id, slug, title, html, short_text AS "shortText", EXTRACT(epoch FROM datetime) * 1000 AS "datetime", thumbnail_url AS "thumbnailUrl", type FROM entries WHERE type = $1 AND slug = $2',
    values: [type, slug]
  }, callback)
}
